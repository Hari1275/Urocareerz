...
async function addTestMenteeOpportunities() {
  try {
    console.log("Adding test mentee opportunities...");

    // First, get a mentee user
    ...
    if (!mentee) {
      console.log("No mentee found. Creating a test mentee...");
      ...
      console.log(`Created test mentee: ${newMentee.email}`);
    }

    const menteeId = mentee ? mentee.id : (await prisma.user.findFirst({
      where: { email: "test.mentee@example.com" },
      select: { id: true }
    })).id;
    ...
    if (opportunityTypes.length === 0) {
      console.log("No opportunity types found. Please run seed-opportunity-types.js first.");
      return;
    }
    ...
    console.log("\nTest mentee opportunities added successfully!");
    ...
    console.log(`\nTotal opportunities for mentee: ${totalOpportunities}`);
